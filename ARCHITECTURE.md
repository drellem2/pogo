...
The first four are about the item. The last is about the machine, and nothing
...
**Why the fourth exists**, given the third. They are complements, not degrees of
...
nothing merged. `--merged-override="<why>"` dispatches anyway; unlike the third
gate's override this has a use that is not a false positive, since an item can
