...
**That is also why a panic in one goroutine is not a local failure — every long-lived goroutine in `internal/agent` and `internal/claude` runs under a guard.** Go tears down the whole process on an unrecovered panic in *any* goroutine, and the paragraphs above say what pogod's death costs: the agents die with it, and a restarted pogod cannot re-adopt them, because re-adoption needs the PTY master fd and that fd died with the process holding it (`internal/agent/orphan.go`). So a panic in one agent's trust hook would not be one agent's problem — it would strand **every** running agent on the host. `internal/agent/gosafe.go` closes that: `GoSafe(site, fn)` replaces bare `go fn()` at every launch site in both packages, recovering the panic, logging the full stack to the daemon's stderr, and emitting a `goroutine_panic` event naming the site. Three properties are deliberate. The guard is **loud** — a recovered panic is still a bug, and swallowing it quietly would trade a visible crash for an invisible one. It is **per-site, not uniform**: `waitAndHandle` carries exit accounting whose last act, closing `a.done`, is the postcondition `Stop`, `StopAll` and every attach connection park on, so a guard that merely recovered and returned would convert one panic into a permanent hang — the callback into foreign code is guarded at the call site instead, and the close is idempotent and deferred. And its completeness is **executable**: `TestGoroutineLaunchesAreGuarded` re-runs the enumeration (every `^\s*go ` launch in the two packages, excluding tests) and fails on any new unguarded one, because "all the ones I found" and "all of them" are different claims. Sweep and rationale: drellem2/pogo#166, which is defence in depth — no panic has ever been observed in pogod.
...
**The fourth clause: a polecat parked at a human gate is reaped without closing its item.** Everything above keys on the item being **terminal**, and one worker in this fleet has an item that is deliberately never allowed to get there. A gh-issue **triage** polecat is instructed not to call `mg done` — no successor exists until the coordinator files the build ticket after the gate, and `mg done` refuses a `declares-remainder` item that names none — so the item stays `claimed` and the reaper's condition can never be satisfied. The polecat is finished, its packet durable on the ticket body, and it holds a worker slot until a person answers. On a `stage: gated` ticket that is unbounded by design: the gate semantics are "silence = HOLD, however long that takes". Measured 2026-08-12 (`p634e` on gh#135, with `mg-0a43` high-priority and five other items undispatchable behind the cap for the whole window; `p1539` and `pc4a9` the same day) and again 2026-09-07 (`t7476` on gh#161, ten issues queued behind a three-slot cap). All four were stopped by hand, and each looked perfectly healthy the entire time.

So the condition is a **disjunction at the top and the same conjunction below**: the item is terminal **or** parked at `stage: gated`, **and** the polecat is quiet, **and** no live reviewer declares it. It is a second way to satisfy "this worker has finished", not a second reaper — a separate detector would re-derive quietness and the review exemption and could reach a different answer about the same polecat on the same tick.

**Releasing that claim is safe only because of the spawn-point stage gate, and the reap keys on the gate's own predicate to keep it that way.** The stop releases the claim, so the item returns to `available/` — and if it were re-dispatched, a second triage polecat's first instructed act is an acknowledgement comment on a stranger's open GitHub issue. `config.IsStageGated` is what stops that, at `handleSpawnPolecat`, and the reap calls the *same* predicate rather than re-implementing "does this stage gate". The two sets are therefore the same set by construction: what this reaps is exactly what the dispatcher refuses. **If that gate is ever narrowed, this reap must be re-argued in the same commit.**

**It cannot reach a builder mid-review, structurally rather than by a check.** The same ticket reports a build polecat idling after PR-open and warns that stopping *that* one is unsafe — it owns an open PR, an unmerged branch, and the modify side of a live review loop, with no equivalent of "the packet is on the ticket". `stage: gated` is carried only by the **triage** ticket: the state carrier assigns the stages to two different tickets (`triage → gated` on the triage ticket, `build → review → merge` on the build ticket), so a builder's item never reads `gated`. The narrow fix stays narrow because the vocabulary is narrow, not because someone remembered to exclude a case. (The wider argument in that ticket — that the per-repo cap counts **workers**, and worker count is not a measure of load, alongside mg-eb47 and gh#128 — is a different accounting question in a different layer, and nothing here settles it.)

**It closes nothing.** The gate reap frees a worker; the item stays exactly where the coordinator parked it, claimed by nobody, awaiting the decision. It does not mark the item, move the stage, or fire the completion notifier — reporting a completion for a decision that has not been made would be a straight falsehood to the one agent waiting on it — and it carries its own `stop_cause` (`gate_reap`, not `done_reap`) so an operator holding the event record can tell "the worker was released" from "the ticket is closed". A probe error leaves the polecat running, which reads the **same** mg-27d4 error the opposite way from the dispatch gate: the gate refuses a spawn on an unreadable carrier because it *may* say `gated`, this declines to reap because it *may not*, and both fail toward the recoverable outcome. That direction is load-bearing here rather than theoretical — a ticket that writes a bare `stage:` line in its own prose parses as unreadable, and this feature's own tickets do exactly that.
