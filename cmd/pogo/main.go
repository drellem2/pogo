...
down. To keep it down, park it.`,
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			if jsonOutput {
				...
				// The health value never prints alone, and the qualification is
				// not optional — see diagnoserender.go.
				diagnoseHealthSection(os.Stdout, diag)
				if diag.TranscriptCheck != nil && diag.TranscriptCheck.State == synthfail.StateUnavailable {
