...
whether it is in flight, already landed, stranded, tracked in a store we cannot
read, or superseded — and act on the answer. It is enabled per PM by adding `"open-prs"` to the `sources` array
...
the listing alone those five states are indistinguishable —

- in flight (a live polecat will land it),
- **landed but open** (the work is already on `main`),
- **stranded** (nothing will ever pick it up),
- **tracked elsewhere** (another fleet is carrying it in a store we cannot
  read — added mg-1f04, after one was reported stranded),
...
### This question has THREE answers, not two

**"Untracked" and "untracked *here*" are different findings, and until mg-1f04
the pass had only one word for both.** The rule below was `not landed +
untracked-in-this-store = STRANDED`, which has **no cell** for a PR tracked in
another fleet's macguffin store — one we cannot read. An unrepresented state
reports as the nearest available one, and here the nearest one is an alarm.

The measurement. `drellem2/macguffin#28` was reported STRANDED: not landed,
tracked by no mg item. Both halves are true **of this box**. Its one review
comment reads
...
`mg-2880` nor `mg-cc4b` resolves here — re-measured 2026-09-08, `mg show` exits
non-zero for both, with `mg show mg-1f04` at exit 0 as the positive control on
the instrument. They are not absent; they are in the **payitgov** fleet's store,
behind a SAML wall our token does not clear (`gh pr list --repo payitgov/agents`
returns *"Resource protected by organization SAML enforcement"*, measured the
same day). The PR was tracked, reviewed and passing.

**And it should still have been reported.** Daniel's ruling on that same PR,
2026-09-08 08:24Z, was *"re macguffin 28 close it, no PRs from payitgov
agents"*. An external-fleet PR is exactly the thing he wants surfaced, so the
obvious repair — downgrade these to informational and stop alarming — is wrong
in the direction that **loses the case**. The sweep's reasoning was wrong and
its outcome was right, and a detector can be both. Refuting a mechanism does not
refute a finding: ask the owner what the finding *means* before softening the
detector that produced it.

So the third answer is **actionable**, with a different sentence and therefore a
different action, and nothing is silenced:

| answer | what was read | what you do |
|---|---|---|
| **tracked here** | an id **this fleet's naming** put on the PR resolves locally | leave it — in flight |
| **tracked elsewhere** | ids are named on the PR, none resolve here, and **none came from this fleet's naming** | *"tracked in a store we cannot read — confirm this PR should exist"* |
| **no tracker found** | nothing named an id, **or** this fleet's naming named one that is not in the store | file a carrier, or establish supersession |

**Mechanical evidence beats prose, and that ordering is the whole safety
property.** An id is read from two kinds of place and they are not equal
evidence: the **head branch and the title** are generated by pogo from the item
it dispatched, so an id there is a claim by *our* naming that an item exists
*here*; the **body and comments** are written by anyone, including another
fleet's reviewer. Read prose first and the known strand goes quiet — gh#93
carries a comment reading *"Triaging as mg-c76a"*, and `mg-c76a` **does** resolve
here (measured 2026-09-08). Read branch and title first and gh#93 stays *no
tracker found*, which is the answer the pass got right the first time. For the
same reason a resolving id found **only** in prose is carried as evidence but
never promoted to *tracked here*.

One limit, stated rather than papered over: `polecat-` branch naming is not
unique to this fleet — `payitgov/macguffin`'s default branch is `polecat-0a5f`.
Another fleet's polecat branch whose id does not resolve here therefore reads
*no tracker found* rather than *tracked elsewhere*. That mis-frames the line; it
does not silence the row, because both answers are actionable and both send a
person to look at the PR.

### The shipped copy of this question

`pogo check-pr-tracking --repo <owner/name>` is this question, executable:

```bash
pogo check-pr-tracking --repo drellem2/pogo --repo drellem2/macguffin
```

It answers **only** this question — landed-ness is the separate predicate below,
and a disposition needs both — and it takes no action: it never opens, closes,
comments on or merges a PR, and never files a work item. Exit 0 nothing
actionable, 1 something is, 2 usage, 3 the resolver's positive control failed.

Two properties worth knowing before you read its zeros:

- **A positive control always runs**, before any repo is read: one id known to
  exist is resolved first, and the command exits 3 rather than reporting if it
  does not come back. Without that, an `mg` that cannot answer makes every
  lookup return "does not resolve" and the sweep reports the whole fleet as
  untracked or externally tracked at once — every row looking measured. The id
  is derived from the store when `--control` is not passed, because a control
  that can be forgotten defaults to the unsafe answer; `--control` only chooses
  which id is used.
- **A repo `gh` could not read is listed separately and makes the run
  actionable**, rather than contributing a reassuring zero. `payitgov/agents` is
  the standing instance. The predicate is `internal/prtracking`, pinned by tests
  against both measured PRs — macguffin#28 must classify *tracked elsewhere* and
  gh#93 must stay a finding.
...
is what reaches a PM's composed prompt.

**The same is true of the tracking rule, and mg-1f04's change to this doc does
not reach it either.** Four of the five TOMLs — `pogo`, `dealdesk`, `onethird`,
`lineara` (`riemann` does not) — carry the two-answer version verbatim, measured
2026-09-08:

    #   2. is it tracked? mg show <id-from-title>  AND  mg list --all | grep <headRefName>
    #   not landed + untracked  -> STRANDED. File a carrier, or establish it is
    #                              superseded.

That is the defective rule itself, in the copy the sweeps actually read. A PM
following it will still report an external-fleet PR as STRANDED no matter what
this file says. **This is worth noticing as a shape and not only as a chore: the
remedy for "an unrepresented state reports as the nearest one" was written into
an artifact the reporting agent does not consult, which is the same defect one
level up.** The correction is three lines and was mailed to `human` with
mg-1f04; the replacement text is

    #   2. is it tracked? pogo check-pr-tracking --repo <slug>
    #      THREE answers, not two — see the doc. `tracked elsewhere` (ids named on
    #      the PR, none resolving here, none from our own naming) is ACTIONABLE:
    #      "tracked in a store we cannot read — confirm this PR should exist".
    #   not landed + no tracker found -> STRANDED. File a carrier, or establish it is
    #                              superseded. A PM reads that every sweep; it reads
...
|---|---|---|
| yes | any | **Rebase-dangle.** The work is on `main` and the PR is noise. Close it and reap the branch at teardown. |
| no | tracked here | **In flight.** Leave it. The work item is the carrier; if it is stalled, that is a stall-watch matter, not this pass. |
| no | **tracked elsewhere** | **External tracker.** *"Tracked in a store we cannot read — confirm this PR should exist."* Route the question to the repo owner naming the ids and the store. **Do not file a carrier** — one exists, somewhere we cannot see — and **do not leave it** either. |
| no | no tracker found | **Stranded.** File a carrier work item, or establish that it is superseded. **Never merge or close on a guess** about whether the change is still wanted. |
| no | superseded | **Superseded.** Close it with a one-line reason naming what landed instead. |

**Row 3 is new (mg-1f04) and it is not a quieter row 4.** It is a *different
question*, put to a different reader: row 4 asks the fleet to pick the work up,
row 3 asks the owner whether the PR should exist at all. Both are findings and
both exit non-zero. The version of this fix that made row 3 informational was
drafted and withdrawn — see [§This question has THREE
answers](#this-question-has-three-answers-not-two) — because the one real
instance of row 3 is the PR Daniel ruled should be **closed**, so a downgrade
would have hidden exactly the case it was built for.
...
*In flight*, *external tracker* and *stranded* are safe under that weaker
reading — not one of the three closes or merges anything — so a `+` that should
have been a `-` costs at most a leave-open, a question to an owner, or a
redundant carrier. **Superseded is not** — it closes a PR, outward-facing, and it is reachable from
a `+` alone, because *stranded* offers "establish that it is superseded" as one
of its two remedies. So supersession is not established by the predicate: before
closing as superseded, confirm on `main` that the *other* change is really there,
the same content check the third constraint below prescribes. `+` on the PR you
are closing tells you nothing either way.

(The rows are named rather than numbered here on purpose. They were numbered
until mg-1f04 inserted one in the middle, at which point every number in this
paragraph pointed at the wrong row and nothing said so.)
...
Four constraints on acting:

- **Never merge or close on a guess.** "Not landed and no tracker found" says
  nobody is carrying the change; it says nothing about whether the change is
  wanted. The two remedies are *file a carrier* and *establish supersession* —
  closing because it looks abandoned destroys the only copy of the work, which is
  precisely gh#93's shape.
- **An external tracker is not a reason to act on the PR yourself, in either
  direction.** *Tracked elsewhere* means the deciding facts — whose work this is,
  whether it was asked for, what the other fleet's item says — are all in the
  store you cannot read. So the remedy is a question to the repo owner, not a
  carrier (one exists) and not a close (you do not know). macguffin#28 is the
  worked example: the answer came back *"close it, no PRs from payitgov
  agents"*, and no amount of reading from this side would have produced it.
