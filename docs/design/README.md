...
| [launchd-reconcile-decision.md](launchd-reconcile-decision.md) | Whether the nightly should RECONCILE the launchd plist drift it reports: the blast radius measured per job, why the "inert install" subset is empty of the jobs worth reconciling, and why writing a plist without reloading it is a regression rather than a partial fix | **Decided: NO** (mg-de0c) — the refusal is enforced by `scripts/pogo-self-deploy_test.sh`; §5 lists what would reopen it |
