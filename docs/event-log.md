...
- **`details` fields:**
  - `category` (string, required): `"unclaimed_items"`, `"unread_mail"`, `"priority_wake"`, `"worked_unclaimed"`, `"preserved_worktree"`, `"stranded_push"`, `"blocked_reminder"`, or `"indefinite_hold"`
...
```
  ...
  - For `preserved_worktree` (mg-836c, mg-fcba): `item_count` (int), `item_ids` ([]string), `worktrees` ([]object — `item_id`, `worktree`, `branch`, `modified_paths` (int), `untracked_paths` (int), `unread` (bool: nothing was established about the tree's contents, so the two counts are not a claim about them), `outcome` (string: `preserved` = uncommitted work positively read, `unpushed` = clean and holding commits that exist nowhere else, `undetermined` = `git status` failed), `commits_finding` (string: `""` none, `local-only`, or `unknown` — the durability question could not be answered, which is reported as itself and is still a finding), `unreachable_commits` (int: how many commits the tree's HEAD reaches that no ref under `refs/remotes/origin/` holds and that have no patch-equivalent on the integration branch; **0 under a `local-only` finding means the list could not be re-read, not that there are none**), `detached` (bool: the tree has no branch, so those commits are held by its HEAD and by no ref at all)), `oldest_age_seconds` (float). The **modified/untracked split** is carried rather than a total because the halves are different facts: a modified tracked file still has its committed version in the object store, while an untracked path is on no branch, in no stash and on no remote — that tree is its only copy on the machine. The **commits fields** are a third such fact and the worst of the three, and they are what make "held because its files are unsaved" and "held because its commits never left the box" countable apart rather than only distinguishable by reading the prose.
  - For `stranded_push` (mg-4bf1): `item_count` (int), `item_ids` ([]string), `branches` ([]object — `item_id`, `branch`, `ref`, `pushed` (bool), `unmerged` (int), `target`, `repo`, `pre_registration` (sha) when the branch carries an unmerged pre-registration commit, and `queued_mr` / `queued_status` when that branch's merge is ALREADY IN THE REFINERY QUEUE), `queue_consulted` (bool), `oldest_age_seconds` (float). **`queued_mr` is what separates two states that are otherwise identical here (mg-64bb)**: a branch pushed and abandoned needs somebody to submit it, a branch in the queue needs everybody to leave it alone, and both are "pushed and unmerged". Counting rows with and without it is how "held because its work was abandoned" and "held because its merge is running" come apart. **`queue_consulted` is the field that keeps a missing `queued_mr` honest**: false means nothing asked the refinery, not that nothing is in flight — and with it false the notice's own remedy may be a submit against a running merge (mg-8baa's collapse, on the remedy rather than the finding). **The category name matches the `work_item_stranded_push` event pogod emits at release on purpose**: that one fires ONCE, when a polecat's claim is released with pushed work behind it, and this one is re-derived from `available/` on every tick for as long as the state lasts. Counting them together is how you tell "detected once and then advertised anyway" from "detected and held". `pushed` is the field that separates two different emergencies and is never folded into the count — pushed work is durable and `pogo refinery submit` will take it, local-only work exists in one worktree that git-gc reaps and the refinery refuses it (mg-586d).
...
- **`details` fields:**
  ...
  - `too_fresh` / `beyond_lookback` / `never_addressed` / `misanchored` / `truncated` (arrays of string, required): the populations declined and why. `never_addressed` is an agent fewer than 2 fires reached — that is `deaf_watch`'s finding and a different remedy, and blaming this agent for it would point the operator at the wrong component. `misanchored` is an agent whose only observed completion **predates its own spawn**: an impossible reading rather than a negative one, since the evidence reader anchors every agent at its own `StartedAt`, so it means the window came from somewhere else and the agent is judged neither dark nor clear (mg-21ad — a mayor with 31 completed turns was mailed about twice as having "completed nothing since it spawned", because its evidence was measured from a crew-mate's spawn 8h17m earlier). `truncated` is an agent whose spawn predates the oldest record the events log still holds: the evidence reader spans the rotated chunks as well as the live log, but rotation eventually discards the oldest, and a count over a window shorter than the agent's life is a floor rather than an answer to "since it spawned" — so it too is judged neither dark nor clear (mg-9d55 — before the reader spanned the rotated files, a rotation inside an agent's lifetime produced this arm's false positive by a second route, at 5,434 instants of this box's recorded history). A report that states its own denominator cannot be misread as coverage it did not have (mg-7a20)
...
- **`details` fields:** `judged`, `scanned`, `too_fresh`, `beyond_lookback`, `never_addressed`, `misanchored`, `truncated`, `grace` — as on `first_turn_watch_dark`; plus `suppressed` (bool, optional), `reason` (string, optional) and `would_have_reported` (array of string, optional) on the settle-window path
...
`dispatch_preserved_worktree_overridden` above). Since mg-fcba that gate also covers the state one step
LATER — committed, not pushed, then stopped, which is what the nightly pre-deploy stop produces — by
asking `gitgc.BranchDurable` about the worktree's own HEAD. The sub-case nothing else can reach is a
tree on a **detached HEAD**: the stranded-work gate does read `refs/heads`, so an unpushed polecat
*branch* has refused a spawn since mg-bfe0, but a detached tree names no ref for any ref scan to find. `~/.pogo/polecats/qbe37` was
