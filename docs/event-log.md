...
(`pogo agent spawn-polecat --merged-override="<why>"`). Unlike its neighbour above
this override has a use that is not a false positive: an item can genuinely owe
work **after** its merge — a release that still has to tag, a change to be
verified in place — and the person dispatching knows that while the gate cannot.
