...
// when the `claude/trust-dialog` key's windowed miss fraction crosses the
...
//
// This package was safe only by arithmetic — a handful of confirmations against
// one too-short-budget miss, under a 0.5 threshold — and the detector is a
// process-global, so `-count=N` accumulates in the same process rather than
// starting over. Adding a deadline-arm test here should not require redoing that
// arithmetic.
//
