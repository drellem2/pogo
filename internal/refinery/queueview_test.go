...
func TestGateWatchMeasuresARealSubtreesCPU(t *testing.T) {
	...
	// compound command, so only a subtree walk finds the CPU. See spinGate.
	busyRecords := runGateArm(t, heartbeat, spinGate(1))
	...
	// "at least one" test.
	idleRecords := runGateArm(t, heartbeat, `sleep 2`)
	...
	// The injected-work arm. Four spinners in place of one, built from the
	// same template so the parallelism is the only thing that changed, and the
	// measurement has to move with it. The arm is capped at the host's core
	// count because N spinners cannot exceed N cores' worth of work; a
	// single-core host cannot inject parallel work at all, and the tracking
	// rule is then dropped rather than weakened.
	...
	if spinners > 1 {
		manyRecords := runGateArm(t, heartbeat, spinGate(spinners))
