...
// in production — a goroutine starved past its budget by a host at load — and Go
// picks uniformly at random among ready select cases, so the scan branch ran
// about half the time per iteration and, because time.After delivers once while
// the ticker keeps firing, nearly always within a few iterations. This package's
// TestLateRenderingDialogIsNeverDismissed failed a merge gate alongside claude's
// and cursor's on exactly that, all three sharing this loop shape.
//
