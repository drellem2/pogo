...
**This is the half the display-label line under "Identity & lifecycle" does not cover, and holding only that half is worse than holding neither.** That line says nothing sets `pogo-crew-<your-name>` on any process, so `pgrep -f pogo-crew-<your-name>` matches nothing even against a healthy agent (mg-710c). True — and a *different* defect with the same symptom. A reader carrying only it reads every empty `pgrep` as a naming artifact, which is exactly the wrong lesson for an ancestry failure, where the empty result means the process was filtered out before the pattern was applied and the agent may be entirely fine or entirely gone. Both are one case each; neither is the whole story (mg-2646).
...
Your agent name is `<your-name>`. Your **display label** is `pogo-crew-<your-name>` — what `pogo agent list` shows and what `/agents` returns as `process_name`. It is **not** a process name: nothing sets it on any process, so `pgrep -f pogo-crew-<your-name>` matches nothing even while you are healthy (mg-710c). That is one reason an empty `pgrep` means nothing, not the only one — the ancestor exclusion above is a different defect with the same symptom, and it fires against patterns that name no display label at all (mg-2646). Ask pogod for an agent's pid. You are started with:
