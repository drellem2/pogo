...
5. **Review through three lenses, in this order.** Each lens produces findings; classify every finding as **blocking** or **advisory** (a nit you explicitly mark non-blocking).
   ...
   1. **QA — build and tests actually run.** Never verdict from reading alone:
      ...
      - **List the brief's "do not X" constraints and check each BY MEASUREMENT.** A precise instruction hands the author the exact words to claim compliance in, and one such claim has sat next to the violation it denied — so the PR's own statement that it complied carries no evidential weight; quote what you measured, not what it claimed. Look hardest where the PR's self-assessment does NOT point: an incomplete self-attack list is the observed failure mode, not a false one.
