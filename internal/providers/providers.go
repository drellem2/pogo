...
// config provider id ("claude", and in future "codex" / "gemini") to its
...
// (internal/claude, …), and those already import internal/agent. Putting the
// resolver in agent would create the cycle agent → claude → agent. Both
// cmd/pogod and cmd/pogo depend on this package so provider selection has a
...
// Resolve maps a config provider id to its agent.Provider descriptor. Claude
// is the only registered provider today; the empty string resolves to Claude
// (the default).
//
