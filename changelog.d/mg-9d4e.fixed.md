- **A work item whose branch has already merged stops being offered for
  ...
  **A fourth dispatch refusal.** `pogo agent spawn-polecat` now refuses an item
  whose work has already landed on the target (409, above every side effect,
  like the three beside it). It is the complement of the stranded-work gate
  ...
  **unmerged**, and this covers the case that opens the moment that branch lands.
  ...
  recorded as `dispatch_merged_work_overridden`; unlike `--stranded-override`
  this has a use that is not a false positive, since an item can genuinely owe
  work after its merge.
  ...
  **And the state stops arising in silence.** Two different things fail the
  ...
  coordinator with the branch, the sha, what `mg` actually said, and the two-command
  remedy. A status probe that FAILS still alerts: an unreadable store is not
  evidence the item closed, and it is the same `mg` that just refused the close.
